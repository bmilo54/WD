...
window.addEventListener('DOMContentLoaded', event => {
    ...
    if (litepickerSingleDate) {
        const minDate = litepickerSingleDate.dataset.minDate || null;  // null if not set
        const maxDate = litepickerSingleDate.dataset.maxDate || null;  // null if not set

        new Litepicker({
            element: litepickerSingleDate,
            format: 'MMM DD, YYYY',
            minDate: minDate,   // will only apply if minDate exists
            maxDate: maxDate    // will only apply if maxDate exists
        });
    }
