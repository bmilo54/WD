...
$(function () {
	const litepickerElements = document.querySelectorAll('.litepickerSingleDate');
	litepickerElements.forEach((element) => {
		new Litepicker({
			element: element,
			format: 'DD/MM/YYYY'
		});
	...
	const startInputs = document.querySelectorAll('.litepickerEventStart');
	startInputs.forEach((startInput) => {
		const group = startInput.dataset.litepickerGroup;
		const endInput = document.querySelector(`.litepickerEventEnd[data-litepicker-group="${group}"]`);
		if (!endInput) return;
		...
		// Initialize start picker with minDate = today (inclusive)
		const startPicker = new Litepicker({
			element: startInput,
			format: 'DD/MM/YYYY',
			minDate: today, // today is selectable
			setup: (picker) => {
				picker.on('selected', (startDate) => {
					// Update end picker min date
					endPicker.setOptions({ minDate: startDate });

					// Get current end date from the picker (as Date object)
					const currentEnd = endPicker.getDate();

					// If end is empty or smaller than start, update it
					if (!currentEnd || currentEnd.getTime() < startDate.getTime()) {
						endPicker.setDate(startDate);
						console.log('End date auto-updated to start date');
					}
				});
			}
		});
	...
	const periodStartInputs = document.querySelectorAll('.litepickerPeriodStart');

	periodStartInputs.forEach((periodStartInput) => {
		const group = periodStartInput.dataset.litepickerGroup;
		const periodEndInput = document.querySelector(`.litepickerPeriodEnd[data-litepicker-group="${group}"]`);
		...
		// Start and End min/max from data attributes
		const minStart = parseDate(periodStartInput.dataset.min);
		const maxStart = parseDate(periodStartInput.dataset.max);
		const minEnd = parseDate(periodEndInput.dataset.min);
		const maxEnd = parseDate(periodEndInput.dataset.max);

		const today = new Date();
		today.setHours(0, 0, 0, 0);

		// Determine initial start value (if already filled from DB)
		const startValueStr = periodStartInput.value;
		const startValue = parseDate(startValueStr);

		// Determine initial end minDate
		const initialEndMin = startValue && (!minEnd || startValue.getTime() > minEnd.getTime())
			? startValue
			: (minEnd || today);

		// Initialize end picker first
		const endPicker = new Litepicker({
			element: periodEndInput,
			format: 'DD/MM/YYYY',
			minDate: initialEndMin,
			maxDate: maxEnd || null,
		});

		// Initialize start picker
		const startPicker = new Litepicker({
			element: periodStartInput,
			format: 'DD/MM/YYYY',
			minDate: minStart || today,
			maxDate: maxStart || null,
			setup: (picker) => {
				picker.on('selected', (startDate) => {
					// Only update endPicker's minDate dynamically after user changes start
					endPicker.setOptions({ minDate: startDate });
...
function initSelect2($element) {
	$element.select2({
		...
		//     ? $element.data('width') 
		//     : ($element.hasClass('w-100') ? '100%' : 'style'),
		placeholder: $element.data('placeholder'),
		allowClear: Boolean($element.data('allow-clear')),
		closeOnSelect: !$element.attr('multiple'),
		minimumResultsForSearch: $element.hasClass('no-search') ? Infinity : 0,
		dropdownAutoWidth: true,
		width: 'resolve',
	});

	// Add custom container class only if you want
	if ($element.hasClass('filter-select')) {
		$element.data('select2').$container.addClass('select2-filter');
	}
}
