...
class UserActivityLog(TimeStampedModel):
    ...
    activity = models.TextField(verbose_name="Activity", max_length=500, blank=True, null=True)

    def __str__(self):
        return '%s - %s' % (timezone.localtime(self.created).strftime('%d-%m-%Y %I:%M %p'), self.user.username)
